...
pub(super) struct FreeIpFinder<Ip, S> {
    ip_batcher: IpBatcher<Ip, S>,
    host_prober: HostProber<Ip>,
}

impl<Ip, S> FreeIpFinder<Ip, S> {
    pub(super) fn new(ip_batcher: IpBatcher<Ip, S>, host_prober: HostProber<Ip>) -> Self {
        Self {
            ip_batcher,
            host_prober,
        }
...
impl<Ip: Clone, S: SelectIp<Ip>> FreeIpFinder<Ip, S> {
    pub(super) async fn find_next(&mut self) -> Option<Vec<Ip>> {
